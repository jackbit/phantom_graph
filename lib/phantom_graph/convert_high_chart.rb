...
module PhantomGraph
  class ConvertHighChart
    ...
    def to_s
      "<PhantomGraph path='#{options[:phantom_js_path]}' highcharts_convert_path='#{options[:highcharts_convert_path]}' config_temp='#{@json_file.path}' image_temp='#{@image_file.path}' phantom_js_command='#{build_cmd}'\njson: #{@json.inspect}\n>"
    end
